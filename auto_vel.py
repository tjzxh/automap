...
def make_map(pcmap,is_circle,width,max_vel_str,max_vel_cur,visible):
    ...
    with open('test.txt', 'r') as f:
        ...
        #calculate the vel of every point
        ...
        #plt.scatter(range(len(all_vel)),all_vel)
        #plt.show()
        ...
        # step3 delete the latter point in every short segment that length is less than 1m
        all_point_final = endpoint.delete_near_point(all_point_final,0.5)
        all_point_final=np.array(all_point_final)
        all_point_final = endpoint.delete_near_point(all_point_final, 1)
        all_point_final = np.array(all_point_final)
        ...
        for i in range(len(lati)):
            ...
            node = {"gps_weight": 1, "id": i, "lat": node_lat, "lng": node_lng, "max_vel": node_vel ,"lslam_carto_weight": 0, "name": str(i),
